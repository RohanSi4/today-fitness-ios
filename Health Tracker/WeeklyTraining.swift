...
struct WeeklyDaySnapshot: Identifiable, Equatable {
    ...
    /// The coach's own one-line summary for the day, e.g. "14 mile long run".
    /// Defaulted so the PostRunDashboard fixture and tests keep compiling.
    var planText: String? = nil

    /// The coach's expanded instructions: effort, heart-rate ceiling, target
    /// pace, venue, routing, fuelling. Generated by `publicPlanDayDetails` on
    /// the coach side and shipped in the plan payload, where they sat unread —
    /// the app has been receiving these the whole time and rendering none of
    ...
    /// Whether there is anything worth opening the row for.
    var hasCoachDetail: Bool {
        !details.isEmpty || !(planText ?? "").isEmpty
    }
